...
void rotate(int animation_delay_up, int animation_delay_down) {
	  /*Вращение туда-сюда*/
	static uint8_t led_mask = 0x01;
	static int btn_state = GPIO_PIN_RESET;
	static int prev_btn_state = GPIO_PIN_RESET;
	static int update_state_downcount = 0;
	...
	if (update_state_downcount < 0) {
	   ...
	   HAL_GPIO_WritePin(GPIOD, led_mask << 12, GPIO_PIN_SET);
	   }
	   HAL_Delay(1);
}
